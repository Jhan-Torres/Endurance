<template>
  ...
  <div class="mt-1 flex flex-col items-center justify-center">
    ...
    <div class="bg-gray-400 py-4 px-6 rounded-lg flex flex-col items-center justify-center m-2 md:py-8 md:px-12 lg:px-14"
      v-if="formButton">
      <fwb-input required placeholder="Title (*)" class="rounded-lg w-52 md:w-96 font-body" v-model="noteTitle"
        v-bind:validation-status="validation">
  ...
  <div class="bg-slate-400 p-1 m-1 rounded-xl flex flex-wrap items-center justify-evenly gap-1 md:gap-3"
    ...
    <fwb-card class="w-80" v-for="note in notesList" :key="note.id">
      <div class="p-2 font-body">
        <h5 class="p-1 text-sm font-bold tracking-wider uppercase rounded-sm bg-gray-700 text-lime-50 md:text-lg">
          {{ note.title }}
        </h5>
        ...
        <font-awesome-icon :icon="['fas', 'file-pen']"
          class="hover:scale-125 hover:cursor-pointer ml-1 transition duration-200 editIcon" />
