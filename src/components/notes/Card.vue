<template>
  ...
  >
    <h5 
      class="p-1 h-12 text-sm font-medium tracking-tight rounded-t-md bg-gray-800 text-lime-50 md:text-base"
    >
      {{ props.noteChild.title }}
    </h5>
    <p 
      class="text-xs text-gray-800 overflow-auto rounded-b-lg text-wrap h-24 bg-gray-200 p-1 md:text-sm"
    >
      {{ props.noteChild.content }}
    </p>
    <hr 
      class="border-gray-300 mx-auto mt-1"
    >
    <button 
    ...
    <h6 
      class="text-sm font-semibold text-teal-500 inline-block float-right uppercase"
    >
