<template>
  <a href="/auth"
    @click.prevent="toggleSession"
    class="font-body text-center group relative inline-block overflow-hidden rounded border-4 bg-gray-300 px-6 py-1 font-bold text-slate-800 hover:text-violet-600 focus:outline-none focus:ring active:bg-indigo-600 active:text-white"
  >
...
const buttonText = computed(() => {
  return (statusSession.value === 'login') ? 'Logout' : 'Login';
});
</script>
